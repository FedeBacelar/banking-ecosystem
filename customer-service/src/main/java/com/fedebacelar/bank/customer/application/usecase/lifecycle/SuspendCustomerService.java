...
public class SuspendCustomerService implements SuspendCustomerUseCase {
    ...
    public SuspendCustomerService(CustomerRepositoryPort customerRepositoryPort, Clock clock) {
        this.customerRepositoryPort = customerRepositoryPort;
        this.clock = clock;
    }

    @Override
    public CustomerDetails suspend(CustomerReasonCommand command) {
        var customer = customerRepositoryPort.findByCustomerId(command.customerId())
                .orElseThrow(() -> new CustomerNotFoundException(command.customerId()));

        return CustomerDetailsMapper.toDetails(customerRepositoryPort.save(customer.suspend(command.reason(), Instant.now(clock))));
    }
}
